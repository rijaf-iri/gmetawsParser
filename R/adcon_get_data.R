get.adcon.data <- function(dirFTP, dirAWS, dirUP = NULL, upload = TRUE){
    on.exit({
    ...
    awsFile <- file.path(dirAWS, "AWS_DATA", "CSV", "adcon_aws_list.csv")
    varFile <- file.path(dirAWS, "AWS_DATA", "CSV", "adcon_parameters_table.csv")
    dirOUT <- file.path(dirAWS, "AWS_DATA", "DATA", "ADCON")
    # dirOUT <- file.path(dirAWS, "AWS_DATA", "DATA0", "ADCON")
    ...
    if(upload){
        ...
        if(inherits(session, "try-error")){
            logUpload <- file.path(dirAWS, "AWS_DATA", "LOG", "ADCON", "processing_adcon.txt")
        ...
        dirADT <- file.path(dirUP, "AWS_DATA", "DATA", "ADCON")
    ...
    for(j in seq_along(awsDIR)){
        ...
        allfiles <- paste0(awsFILES[j], '-', daty, '.csv')
        ...
        out <- lapply(seq_along(allfiles), function(i){
            ...
            qres <- try(read.adcon.table(file_csv), silent = TRUE)
            ...
            aws.dat <- try(parse.adcon.data(qres, awsID[j], varTable), silent = TRUE)
