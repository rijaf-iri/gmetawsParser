...
get.adcon.data_aws <- function(conn, dirAWS, dirUP = NULL, upload = TRUE){
    on.exit({
    ...
    awsFile <- file.path(dirAWS, "AWS_DATA", "CSV", "adcon_db_rain_aws_list.csv")
    varFile <- file.path(dirAWS, "AWS_DATA", "CSV", "adcon_db_rain_parameters_table.csv")
    # dirOUT <- file.path(dirAWS, "AWS_DATA", "DATA0", "ADCON_AWS")
    dirOUT <- file.path(dirAWS, "AWS_DATA", "DATA", "ADCON_AWS")
    ...
    if(upload){
        ...
        if(inherits(session, "try-error")){
            logUpload <- file.path(dirAWS, "AWS_DATA", "LOG", "ADCON_AWS", "processing_adcon_aws.txt")
        ...
        dirADT <- file.path(dirUP, "AWS_DATA", "DATA", "ADCON_AWS")
    ...
    for(j in seq_along(awsID)){
        ...
        ## adcon raingage and agro network
        qres$network <- 2
